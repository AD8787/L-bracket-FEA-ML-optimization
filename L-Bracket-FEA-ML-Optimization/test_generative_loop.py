...
valid_designs = candidates_df[candidates_df["Predicted_Safety_Factor"] >= target_safety_factor].copy()
optimal_design = valid_designs.sort_values(by = "Thickness_mm").iloc[0]

print(f"Optimal Bracket Design for Target Safety Factor of {target_safety_factor}:")
print(f"- Target Safety Factor: {target_safety_factor}")
